/**
 * Fix 5/29 ?? deposit 1,140,040: two per-sale vouchers + matchAutoLinked:false (????).
 * Usage: npx tsx scripts/repair-urim-may29-deposit.mts [sqlite-path] [--dry-run]
 */
...
const EXPECTED_SALES_IDS = ["3590", "3597"];
...
const candidate = {
  ...
  score: 100,
  reasons: ["repair-urim-may29-deposit"],
...
if (vouchers.length !== 2) {
  console.error(`Expected 2 vouchers, got ${vouchers.length}`);
  process.exit(1);
}
...
const logs = vouchers.map((voucher, index) => ({
  ...
  savedBy: "repair-urim-may29-deposit",
...
state.bankTransactions = (state.bankTransactions || []).map((row) =>
  row.id === TX_ID
    ? {
        ...
        matchConfirmedBy: "repair-urim-may29-deposit",
        matchAutoLinked: false,
      }
    : row,
);

const saved = saveErpState(state, version, "repair-urim-may29-deposit");
