...
import {
  ...
  tryRuleBasedTaxInvoiceBankLinkView,
  ...
  formatTaxInvoiceBankLinkViewAnswer,
  buildChatActionsFromTaxInvoiceBankLinkView,
...
const SYSTEM_PROMPT = [
  ...
  "\uC2DC\uACF5\uC790 \uCC28\uB7C9\uBC88\uD638(\'\uCC28\uBC88\uD638\', \'\uCC28\uB7C9 \uBC88\uD638\' \uD3EC\uD568)\uB294 get_worker_info\uB97C \uC0AC\uC6A9\uD558\uC138\uC694. \uC2DC\uACF5\uC790/\uAC70\uB798\uCC98 \uACC4\uC88C(\uC740\uD589/\uACC4\uC88C\uBC88\uD638) \uC870\uD68C\uB294 get_person_bank_account \uB3C4\uAD6C\uB97C \uC0AC\uC6A9\uD558\uC138\uC694. '\uAC15\uD0DC\uC6D0 \uACC4\uC88C\uBC88\uD638' \uCC98\uB7FC \uC774\uB984+\uACC4\uC88C \uC694\uCCAD\uC740 \uB85C\uADF8\uC778 \uC0AC\uC6A9\uC790\uC758 ERP \uC5C5\uBB34 \uBC94\uC704\uC774\uBBC0\uB85C \uAC70\uBD80\uD558\uC9C0 \uB9D0\uACE0 \uB3C4\uAD6C \uACB0\uACFC\uB97C \uADF8\uB300\uB85C \uB2F5\uD558\uC138\uC694.",
  ...
  "\uD1B5\uC7A5 \uBBF8\uC5F0\uACB0/\uC5F0\uACB0 \uACC4\uC0B0\uC11C \uBCF4\uAE30(\uC608: \uD1B5\uC7A5\uC5F0\uACB0 \uC548\uB41C \uACC4\uC0B0\uC11C \uBCF4\uC5EC\uC918, \uD1B5\uC7A5 \uBBF8\uC5F0\uACB0 \uACC4\uC0B0\uC11C)\uC740 navigate_erp target=accounting_tax \uC640 taxBankLinkFilter=unlinked \uB610\uB294 linked \uB97C \uC801\uC6A9\uD558\uC138\uC694. \uAE30\uAC04 \uC5C6\uC774 \uC694\uCCAD\uD558\uBA74 \uC804\uCCB4 \uAE30\uAC04\uC785\uB2C8\uB2E4.",
...
function tryRuleBasedOpenFromQuestion(question) {
  ...
  const taxBankLinkViewResult = tryRuleBasedTaxInvoiceBankLinkView(question);
  if (taxBankLinkViewResult) {
    return {
      answer: formatTaxInvoiceBankLinkViewAnswer(taxBankLinkViewResult),
      chatActions: buildChatActionsFromTaxInvoiceBankLinkView(taxBankLinkViewResult),
    };
  }
...
export async function handleErpChat({ messages, user: tokenUser }) {
  ...
  if (taxNavUsed?.result?.nav?.taxBankLinkFilter) {
    const { taxBankLinkFilter, startDate, endDate } = taxNavUsed.result.nav;
    answer = formatTaxInvoiceBankLinkViewAnswer({
      ok: true,
      bankLinkFilter: taxBankLinkFilter,
      startDate,
      endDate,
      periodLabel: startDate && endDate ? `${startDate}~${endDate}` : undefined,
      periodKey: startDate && endDate ? "custom" : "all",
    });
    chatActions = buildChatActionsFromTaxInvoiceBankLinkView({
      ok: true,
      bankLinkFilter: taxBankLinkFilter,
      startDate,
      endDate,
    ...
    taxNavHandled = true;
  } else if (taxNavUsed?.result?.nav?.taxSearchQuery) {
