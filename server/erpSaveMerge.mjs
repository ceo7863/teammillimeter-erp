...
/**
 * Generic ERP saves must NEVER create/update/delete receipts.
 * Dedicated /api/receipts* uses saveErpState(..., { allowReceiptMutation: true }).
 * This merge always preserves existing authoritative ledger rows.
 */
export function mergeReceiptsForSave(existing = [], _incoming = []) {
  return [...(existing || [])];
}

export function mergeReceiptAllocationsForSave(existing = [], _incoming = [], _receipts = []) {
  return [...(existing || [])];
}
