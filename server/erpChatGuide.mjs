...
import { formatChatGuideTextAnswer } from "./erpChatGuideContent.mjs";
...
const CHAT_GUIDE_PDF_QUERY_PATTERN =
  /\uAC00\uC774\uB4DC.*pdf|pdf.*\uAC00\uC774\uB4DC|pdf.*\uC5EC|\uC5EC.*pdf|pdf.*\uCD9C\uB825|pdf.*\uBC1B|\uC0AC\uC6A9(?:\uBC95|\uAC00\uC774\uB4DC).*pdf|pdf.*\uC0AC\uC6A9(?:\uBC95|\uAC00\uC774\uB4DC)/i;

export function isChatGuidePdfQuery(message) {
  const raw = String(message || "").trim();
  if (!raw) return false;
  const normalized = raw.replace(/\s+/g, "");
  return CHAT_GUIDE_PDF_QUERY_PATTERN.test(normalized);
}

export function isChatGuideQuery(message) {
  const raw = String(message || "").trim();
  if (!raw) return false;
  if (isChatGuidePdfQuery(raw)) return false;
  const normalized = raw.replace(/\s+/g, "");
  return CHAT_GUIDE_QUERY_PATTERN.test(normalized);
}

export function formatChatGuideAnswer() {
  return formatChatGuideTextAnswer();
}

export function formatChatGuidePdfAnswer() {
  return [
