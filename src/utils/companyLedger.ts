...
export function isFixedExpensePaymentBankLinked(
  payment: FixedExpensePayment,
  bankTransactions: Array<{ id?: string; linkedFixedExpensePaymentId?: string }> = [],
) {
  if (Boolean(String(payment.bankTransactionId || "").trim())) return true;
  return bankTransactions.some((tx) => tx.linkedFixedExpensePaymentId === payment.id);
}

function normalizeFixedExpenseMatchKey(text: string) {
  return String(text || "")
    .toLowerCase()
    .replace(/\s+/g, "")
    .replace(/[^\w가-힣]/g, "");
...
function resolveBankTransactionForFixedPayment(
  ...
  if (directId) {
    return bankTransactions.find((tx) => tx.id === directId) || null;
  }
  return bankTransactions.find((tx) => tx.linkedFixedExpensePaymentId === payment.id) || null;
}

/** 통장 연결됐거나, 같은 달·같은 항목/금액으로 이미 다른 납부가 연결된 경우 */
export function isFixedExpensePaymentSettled(
  payment: FixedExpensePayment,
  payments: FixedExpensePayment[],
  ...
  const expenseKey = normalizeFixedExpenseMatchKey(expense?.name || "");
  const roomKey = extractFixedExpenseRoomKey(`${expense?.name || ""} ${expense?.category || ""}`);

  if (
    payments.some(
      (row) =>
        row.id !== payment.id &&
        row.fixedExpenseId === payment.fixedExpenseId &&
  ...
  return payments.some((row) => {
    if (row.id === payment.id) return false;
    if (getMonthKey(row.date) !== monthKey) return false;
    if (!isFixedExpensePaymentBankLinked(row, bankTransactions)) return false;
    if (Number(row.amount) !== amount && !areRecurringAmountsCompatible(Number(row.amount), amount)) return false;

    const otherExpense = fixedExpenses.find((item) => item.id === row.fixedExpenseId);
    const otherKey = normalizeFixedExpenseMatchKey(otherExpense?.name || "");
    const otherRoom = extractFixedExpenseRoomKey(`${otherExpense?.name || ""} ${otherExpense?.category || ""}`);

    if (roomKey && otherRoom && roomKey === otherRoom) return true;
    if (expenseKey.length >= 2 && otherKey.length >= 2) {
      if (expenseKey.includes(otherKey) || otherKey.includes(expenseKey)) return true;
    }

    const tx = resolveBankTransactionForFixedPayment(row, bankTransactions);
    if (!tx) return false;

    const haystack = normalizeFixedExpenseMatchKey(
      [tx.description, tx.counterpartyName, tx.memo, row.memo].filter(Boolean).join(" "),
    );
    return expenseKey.length >= 2 && haystack.includes(expenseKey);
  });
}

export function isAutoGeneratedFixedExpensePayment(payment: FixedExpensePayment) {
  return String(payment.memo || "").includes("\uC790\uB3D9 \uB4F1\uB85D");
}

export function pruneSettledDuplicateFixedExpensePayments(input: {
  fixedExpensePayments: FixedExpensePayment[];
  bankTransactions: Array<{
    id?: string;
    linkedFixedExpensePaymentId?: string;
  ...
  const payments = input.fixedExpensePayments;
  const removeIds = new Set<string>();

  for (const payment of payments) {
    if (isFixedExpensePaymentBankLinked(payment, input.bankTransactions)) continue;
    if (!isAutoGeneratedFixedExpensePayment(payment)) continue;
    if (
      ...
      removeIds.add(payment.id);
    }
  }

  if (!removeIds.size) {
    return { payments, removedCount: 0, removedIds: [] as string[] };
  }
