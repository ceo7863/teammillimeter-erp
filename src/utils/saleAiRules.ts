...
export function buildSaleAiRuleFormPreview(
  ...
  options: {
    sampleHours?: number;
    sampleWorkerCharge?: number;
    capSampleHours?: number;
    paySampleHours?: number;
    sampleWorkerPay?: number;
    manwonSampleHours?: number;
  } = {},
): SaleAiRuleFormPreview {
  const sampleHours = options.sampleHours ?? 2;
  const capSampleHours = options.capSampleHours ?? 4;
  const paySampleHours = options.paySampleHours ?? 4;
  const manwonSampleHours = options.manwonSampleHours ?? 1.5;
  const sampleWorkerCharge = options.sampleWorkerCharge ?? 250000;
  const sampleWorkerPay = options.sampleWorkerPay ?? 200000;
