...
export const WORKER_AUDIT_FIELDS: AuditFieldDef[] = [
  ...
  { key: "probationAdjustedAt", label: "수습 조정일" },
  { key: "probationNetPay", label: "수습 월실지급", format: (v) => formatAuditMoney(v) },
  {
    key: "probationPayWithVat",
    label: "수습 부가세 포함",
    format: (v) => (v === true ? "포함" : v === false ? "미포함" : "전역 규칙"),
  },
  { key: "postProbationConstructionCost", label: "수습 종료 시공비", format: (v) => formatAuditMoney(v) },
  { key: "postProbationCustomChargeCost", label: "수습 종료 개별청구단가", format: (v) => formatAuditMoney(v) },
  { key: "postProbationGrade", label: "수습 종료 등급" },
  { key: "isActive", label: "상태", format: (v) => (v === false ? "비활성" : "활성") },
  { key: "bank", label: "은행명" },
